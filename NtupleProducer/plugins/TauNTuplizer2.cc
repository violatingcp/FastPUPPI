...
//#include "DataFormats/Phase2L1ParticleFlow/interface/L1PFTau.h"
...
class TauNTuplizer2 : public edm::one::EDAnalyzer<edm::one::SharedResources,edm::one::WatchRuns>  {
    public:
        explicit TauNTuplizer2(const edm::ParameterSet&);
        ~TauNTuplizer2();
    ...
    private:
        ...
        edm::EDGetTokenT<std::vector<l1t::L1PFTau>> L1PFTaus_;
...
TauNTuplizer2::TauNTuplizer2(const edm::ParameterSet& iConfig) :
  L1PFTaus_    (consumes<std::vector<l1t::L1PFTau>>(iConfig.getParameter<edm::InputTag>("src"))),
...
TauNTuplizer2::~TauNTuplizer2() { }
void  TauNTuplizer2::beginJob() {
    mc_.makeBranches(tree_);
    for (auto & v : reco_) v.makeBranch(tree_);
    eventcount_=0;
}
void TauNTuplizer2::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    run_  = iEvent.id().run();
...
DEFINE_FWK_MODULE(TauNTuplizer2);
